...
class InitialState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var desktopFind:Process = new Process("powershell \"(New-Object -ComObject Shell.Application).NameSpace('shell:Desktop').Self.Path\"");
		var desktopPath:String = desktopFind.stdout.readLine();
		desktopFind.close();

		var directoryCheck:Bool = sys.FileSystem.exists(desktopPath + "/IMSCARED/FNF");
		trace("Dir check returned : " + directoryCheck);

		if (directoryCheck != true)
		{
			sys.FileSystem.createDirectory(desktopPath + "/IMSCARED/FNF");
		...
		if (before)
		{
			warnText = new FlxText(0, 0, FlxG.width, "You've been here before haven't you?", 32);
